...
SCServo::SCServo(std::string _serialPort) {
	...
	if(serial_ == nullptr){
		if(this->init(_serialPort)){
			serial_ = serialPorts_[_serialPort];
		}
	}
...
void SCServo::writeBuf(u8 ID, u8 MemAddr, u8 *nDat, u8 nLen, u8 Fun) {
	try {
		...
		if(nDat){
			...
			bBuf[3] = msgLen;
			bBuf[5] = MemAddr;
			serial_->write(bBuf, 6);		
		}else{
			bBuf[3] = msgLen;
			serial_->write(bBuf, 5);		
		}
		...
		serial_->write(nDat, nLen);
		u8 negCheckSum = ~CheckSum;
		serial_->write(&negCheckSum, 1);	
	}catch (std::exception& e){
		std::cerr << e.what() << std::endl;
	}		
}
...
void SCServo::syncWrite(u8 ID[], u8 IDN, u8 MemAddr, u8 *nDat[], u8 nLen) {
	try {
		...
		bBuf[5] = MemAddr;
		bBuf[6] = nLen;
		serial_->write(bBuf, 7);

		Sum = 0xfe + mesLen + INST_SYNC_WRITE + MemAddr + nLen;
		u8 i, j;
		for(i=0; i<IDN; i++){
			serial_->write(&ID[i],1);
			serial_->write(nDat[i], nLen);
...
int SCServo::Read(u8 ID, u8 MemAddr, u8 *nData, u8 nLen) {
	try {
		...
		u8 bBuf[5];
		if(serial_->read(bBuf, 5)!=5){
			return 0;
		}
		int size = serial_->read(nData, nLen);
		if(serial_->read(bBuf, 1)){
			return size;
		}		
	}catch (std::exception& e){
		std::cerr << e.what() << std::endl;
...
std::map<std::string, serial::Serial*> SCServo::serialPorts_;
